...
use crate::server::{controllers::answer::{self, Answer}, services::services_writing::{get_singular_writing, save_writing}};
...
pub fn handle_non_post_writings_request(method: &str, path_segments: &[&str], body: &str) -> Answer {
    match method {
        "GET" => {
            if path_segments.len() == 1 {
                Answer::new(200, "Around the fur".to_string())
            } else if path_segments.len() == 2 {
                let id = path_segments[1];
                let writing_result = get_singular_writing(id.to_owned());
                let fields_separator = "###";
                let fields_separator_2 = "¬¬¬";
                let answer: Answer = match writing_result {
                    Ok(writing) => {
                        let mut response_tags = String::new();
                        
                        for tag in writing.tags {
                            response_tags.push_str(
                                &format!("{},{}",tag.id,tag.name)
                            );
                        }

                        let mut response_comments = String::new();

                        for comment in writing.comments {
                            response_comments.push_str(
                                &format!("{}{}{}{}{}{}",
                                comment.id, fields_separator_2, comment.author, fields_separator_2, comment.content, fields_separator_2
                            )
                            );
                        }
                        ...
                        let response_body = format!(
                            "{}{}{}{}{}{}{}{}{}{}{}{}{}", 
                            writing.id, fields_separator, writing.author, fields_separator, writing.title, fields_separator, 
                            writing.image, fields_separator, writing.content, fields_separator, response_comments, fields_separator, response_tags);
                        
                        Answer::new(200, response_body)
                    },
                    Err(e) => Answer::new(500, format!("Error fetching a single writing: {}", e))  
                };
                answer
                
            } else {
                Answer::new(400, "Improper request for fetching one writing".to_string())
            }
        },
        "DELETE" => {
            Answer::new(400, "Improper request for fetching one writing".to_string())
